...
export interface SupermercadosSimulatorProps {
...
export { type FourLevelsCatalog, type CatalogEntity }
...
function promoMatchesRequirements(promo: SupermarketPromoItem, userMethods: string[]): boolean {
  ...
  return promo.requirements.some(r => {
    ...
    if (hasBankReq && hasWalletReq) return userHasBank && userHasWallet && userHasNetwork
    if (hasBankReq) return userHasBank && userHasNetwork
    if (hasWalletReq) return userHasWallet && userHasNetwork
    if (hasNetworkReq) return userMethods.some(m => matchesMethod(m, r.cardNetworkSlug, r.cardNetworkName))
    return true
  })
}
...
export default function SupermercadosSimulator({
  ...
  const results: SupermarketResultItem[] = useMemo(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full">
          ...
          <div className="fixed inset-0 z-50 lg:hidden flex">
            ...
            <div className="relative ml-auto w-full max-w-xs h-full bg-slate-900 z-10 shadow-2xl flex flex-col">
              <TreeFilterSidebar
                ...
                todayInfo={todayInfo}
                callbackUrl="/ahorro-interactivo/supermercados"
                isMobileDrawer={true}
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <main className="flex-1 min-w-0 w-full space-y-4">
            ...
            {/* Grid de Cadenas de Supermercados */}
      ...
      <footer className="w-full bg-[#060D1A] border-t border-slate-800/80 text-slate-400 py-12 px-4 mt-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="border-t border-slate-800/80 pt-6 text-center text-xs text-slate-500">
            <p>© {new Date().getFullYear()} PromoAR. Las promociones son provistas por cada entidad financiera y cadena de supermercados. Verificá términos, vigencia y exclusiones antes de comprar.</p>
          </div>
        </div>
      </footer>
    </div>
