          ...
          <div className="space-y-6">
            <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl w-fit">
              {entities?.categories.map(cat => (
                <button
                  key={cat.id}
                  onClick={() => { setSubTab(cat.id); setFilterText(''); setFilterCommerce('') }}
            ...
            <div className="flex gap-3 items-center">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  type="text"
                  placeholder="Buscar por título o comercio..."
                  value={filterText}
              ...
              >
                ...
                {Array.from(new Set(promos.filter(p => p.categoryId === subTab && p.status !== 'EXPIRED').map(p => p.commerce?.name).filter(Boolean))).sort().map(name => (
                  <option key={name} value={name}>{name}</option>
                ))}
              </select>
              {(filterText || filterCommerce) && (
                <button onClick={() => { setFilterText(''); setFilterCommerce('') }} className="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1">
                  <X size={12} /> Limpiar
                </button>
              )}
              <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-3 py-1.5 rounded-full ml-auto">
                {promos.filter(p => {
                  if (p.categoryId !== subTab || p.status === 'EXPIRED') return false
