...
import type { Reco, RecommendationResponse } from '@/lib/useRecommendations'
...
// Hero personalizado — primer viewport de la Home. Entrega valor concreto
// (no un banner) en <3s de lectura: cuántas oportunidades hay hoy + la más
  ...
  if (loading && !data) {
    return (
      <div className="px-4 pt-3 pb-5">
        ...
        <div className="rounded-3xl bg-gray-50 dark:bg-slate-800/60 border border-gray-100 dark:border-slate-700 p-5 h-[168px] animate-pulse" />
      </div>
    )
  }
    ...
    return (
      <div className="px-4 pt-3 pb-5">
        ...
        <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">
          Mirá el listado completo más abajo — puede haber algo para otro día de la semana.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="px-4 pt-3 pb-5">
      <h1 className="text-[21px] font-black text-[#0D1B2E] dark:text-white leading-tight mb-4">
        {count === 1
          ? 'Hoy tenés una oportunidad que vale la pena'
          : <>Hoy tenés <span className="text-[#E8471C]">{count} oportunidades</span> que valen la pena</>}
      </h1>

      <button
        onClick={handleClick}
        className="w-full text-left rounded-3xl bg-white dark:bg-[#0F2040] border border-[#E4E8EF] dark:border-slate-700 p-4 flex items-center gap-4 hover:border-[#1D3D6E]/40 dark:hover:border-[#3A6BC4]/50 hover:shadow-[0_8px_24px_rgba(29,61,110,0.12)] transition-all group"
      >
        <div
        ...
        <div className="min-w-0 flex-1">
          ...
          {num && (
            <p className="flex items-baseline gap-1 mt-0.5">
              <span className="text-[26px] font-black text-[#1D3D6E] dark:text-[#8AADD4] leading-none tabular-nums">{num}{unit}</span>
              <span className="text-[12px] text-slate-400 dark:text-slate-500">{label}</span>
            </p>
          )}
          {top.reasons?.[0] && (
            <p className="text-[11px] font-semibold text-[#1D3D6E] dark:text-[#8AADD4] mt-1 flex items-center gap-1">
