...
export default function CategorySheet({ isOpen, onClose, categorias, selected, onChange }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col justify-end lg:justify-center lg:items-center">
      ...
      <div className="relative bg-white rounded-t-[32px] lg:rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-300 max-h-[80vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-black text-gray-900">Categorías</h2>
            {selected.length > 0 && (
            ...
            {sorted.map(cat => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[11px] font-bold flex-1 text-left leading-tight truncate ${isSelected ? 'text-indigo-700' : 'text-gray-700'}`}>
                    {cat.name}
                  </span>
                  {cat.promoCount != null && cat.promoCount > 0 && (
                    <span className={`text-[10px] font-black shrink-0 ${isSelected ? 'text-indigo-400' : 'text-gray-400'}`}>
                      {cat.promoCount}
                    </span>
                  )}
                  {isSelected && (
        ...
        {/* Footer */}
        <div className="px-4 pb-5 pt-3 border-t border-gray-100 flex gap-2">
          <button
            onClick={() => onChange([])}
            disabled={selected.length === 0}
            className="flex-1 py-3 rounded-2xl border border-gray-200 text-gray-500 font-bold text-sm hover:bg-gray-100 transition-colors disabled:opacity-40"
          >
            Limpiar
          </button>
          <button
            onClick={onClose}
            className="flex-[2] py-3 rounded-2xl bg-gray-900 text-white font-bold text-sm shadow-lg hover:bg-gray-800 transition-all"
          >
