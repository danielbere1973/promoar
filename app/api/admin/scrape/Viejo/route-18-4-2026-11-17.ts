...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const [, group] of grouped) {
      ...
      console.log(`[Route DEBUG ALL] "${p.title}" → discount: ${p.discount}, bankNames: ${p.bankNames?.length || 0}, walletNames: ${p.walletNames?.join(',') || 'ninguna'}`);
      if (!p.title) continue;
      ...
      console.log(`[REQ DEBUG] "${p.title}" → bankIds: ${JSON.stringify(resolvedBankIds)}, walletIds: ${JSON.stringify(resolvedWalletIds)}`);

      const existing = p.sourceUrl
        ? await prisma.promo.findFirst({ where: { sourceUrl: p.sourceUrl, title: p.title } })
