// Este es el route.ts ORIGINAL antes de agregar manejo de múltiples redes
// Solo maneja UNA red de tarjeta (cardNetworkName) como string
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const p of flatPromos) {
      ...
      // Red de tarjeta (singular, string)
      const netMatch = p.cardNetworkName
        ? cardNetworks.find(cn =>
          normalizeStr(cn.name).includes(normalizeStr(p.cardNetworkName!))
        )
        : null;
