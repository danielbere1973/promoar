...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const p of flatPromos) {
      ...
      // ── NUEVO: Redes de tarjeta con tipo (crédito/débito) ─────────────────────
      ...
      if (p.cardNetworks && p.cardNetworks.length > 0) {
        for (const item of p.cardNetworks as CardNetworkWithType[]) {
          ...
          const netMatch = cardNetworks.find(cn =>
            ...
            // Matching específico para American Express
            (normalizeStr(networkName) === 'american express' && normalizeStr(cn.name).includes('american'))
          );
        ...
        // Si no hay redes, crear un requirement sin red específica
      ...
      if (p.bankNames && p.bankNames.length > 0) {
        for (const bankItem of p.bankNames) {
          ...
          // FIX: Buscar por bcraCode O codigoModo (tabla banks tiene ambos)
      ...
      // ── NUEVO: Requirements (banco × wallet × (red + tipo)) ───────────────────
      const reqData: any[] = [];
