...
type FinancialProduct = {
  ...
  lastFour?:      string | null   // últimos 4 dígitos de tarjeta
  accountNumber?: string | null   // CBU completo (banco+cuenta) o CVU completo (billetera)
  shortAccountNumber?: string | null // El número de cuenta corto
  bankAccountType?:    string | null // CA o CC
  currency?:           string | null // ARS o USD
  alias?:              string | null // Alias de CBU o CVU
  isPayroll?:          boolean
  isPensioner?:        boolean
}
type Profile = {
...
function productIcon(p: FinancialProduct) {
  if (p.wallet)                 return { bg: 'bg-purple-50', text: 'text-purple-600', Icon: Wallet }
  if (p.cardType === 'ACCOUNT') return { bg: 'bg-blue-50',   text: 'text-blue-600',   Icon: Building2 }
  return                               { bg: 'bg-orange-50', text: 'text-orange-600', Icon: CreditCard }
}

// Clases reutilizables
const inp = 'w-full bg-gray-50 border border-gray-100 text-sm font-medium px-4 py-3 rounded-2xl focus:bg-white focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all outline-none'
const sel = 'w-full bg-gray-50 border border-gray-100 text-sm font-medium px-4 py-3 rounded-2xl focus:bg-white outline-none appearance-none'
const inpInd = 'w-full bg-gray-50 border border-gray-100 text-sm font-medium px-4 py-3 rounded-2xl focus:bg-white focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 transition-all outline-none'
const selInd = 'w-full bg-gray-50 border border-gray-100 text-sm font-medium px-4 py-3 rounded-2xl focus:bg-white focus:border-indigo-400 outline-none appearance-none'

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide ml-1 mb-1.5 block">{label}</label>
      {children}
    </div>
  )
}
...
export default function PerfilPage() {
  ...
  const [productSegments, setProductSegments] = useState<BankSegment[]>([])
  ...
  const emptyProduct = {
    ...
    cardType:      'CREDIT', 
    networks:      [] as { id: string; modo: boolean }[],  // ← ahora guardamos el ID y si está en MODO
    lastFour:      '',
    accountNumber: '',
    shortAccountNumber: '',
    bankAccountType: 'CA',
  ...
  function resetForm() { setNp(emptyProduct); setProductSegments([]); setProductNetworks([]) }

  const toggleNetwork = (id: string) => {
    setNp(n => {
      const exists = n.networks.find(x => x.id === id)
      if (exists) {
        return { ...n, networks: n.networks.filter(x => x.id !== id) }
      } else {
        return { ...n, networks: [...n.networks, { id, modo: false }] }
      }
    })
  }

  const toggleNetworkModo = (e: React.MouseEvent, id: string) => {
    e.stopPropagation() // evitar que se desmarque la tarjeta al tocar el icono de MODO
    setNp(n => ({
      ...n,
      networks: n.networks.map(x => x.id === id ? { ...x, modo: !x.modo } : x)
    }))
  }
  ...
  function handleEntityChange(entityId: string) {
    ...
    if (bank) {
      ...
      setProductSegments(bank.segments || [])
  ...
  function handleEditProduct(p: FinancialProduct) {
    ...
    setNp({
      ...
      cardType:    p.cardType === 'ACCOUNT' ? 'CREDIT' : p.cardType,
      cardNetworkIds: [], // no se usa más, usamos networks
      networks: p.cardNetwork?.id ? [{ id: p.cardNetwork.id, modo: isModo }] : [],
      lastFour:           p.lastFour        || '',
  ...
  async function handleAddProduct() {
    const cardType = np.productType === 'ACCOUNT' ? 'ACCOUNT' : np.cardType

    if (np.productType === 'CARD') {
      // Creación de múltiples tarjetas
      await Promise.all(np.networks.map(async (net) => {
        const payload = {
          cardId:        editingId || undefined,
          cardBankId:    np.bankId        || undefined,
          cardWalletId:  (net.modo && modoWallet) ? modoWallet.id : null,
          segmentId:     np.segmentId     || undefined,
          cardType,
          cardNetworkIds: [net.id],
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      {/* Header */}
      <div className="bg-white px-5 py-5 sticky top-0 z-10 shadow-sm shadow-black/[0.02]">
        ...
        <div className="flex bg-gray-100 p-1 rounded-2xl">
          {(['personal', 'finance'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-xl transition-all ${activeTab === tab ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
        ...
        <div className="mt-4">
          <div className="flex justify-between items-center mb-1.5">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Completitud del perfil</span>
      ...
      <div className="px-5 py-6 pb-32 max-w-lg mx-auto">
        {loading ? (
          <div className="space-y-4 py-10">
            {[1,2,3].map(i => (
              <div key={i} className="bg-white border border-gray-100 rounded-3xl p-5 animate-pulse">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {/* Email read-only */}
            <div className="bg-white border border-gray-100 rounded-3xl px-6 py-5 shadow-sm">
              <div className="flex items-center gap-2 mb-1.5">
                <Mail size={13} className="text-gray-400" />
                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Correo electrónico</span>
              </div>
            ...
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-5">Identificación</h2>
            ...
            {/* Contacto */}
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-5">Contacto</h2>
              <div className="space-y-4">
                <Field label="Celular">
            ...
            {/* Domicilio */}
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-5">Domicilio</h2>
              <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="font-bold text-gray-900 text-sm">Mis Productos Financieros</h2>
                    <p className="text-[10px] text-gray-400 mt-0.5">Tarjetas, cuentas y billeteras</p>
                  </div>
                </div>
                <span className="text-xs font-bold text-gray-400 bg-gray-50 px-2.5 py-1 rounded-full">{profile?.cards.length ?? 0}</span>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                {(profile?.cards.length ?? 0) === 0 && !addingProduct && (
                  <div className="py-8 text-center">
                    ...
                    <p className="text-sm font-bold text-gray-400">Todavía no agregaste productos</p>
                    <p className="text-xs text-gray-400 mt-1">Agregá tu primera tarjeta o cuenta bancaria</p>
                  </div>
                )}
                ...
                {profile?.cards.map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-4 bg-gray-50/80 border border-gray-100 px-4 py-3.5 rounded-2xl hover:bg-white hover:shadow-sm transition-all">
                      <div className={`${bg} ${text} p-2.5 rounded-xl shrink-0`}><Icon size={18} /></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-900">{entity}</p>
                        <p className="text-[11px] text-gray-500 font-medium mt-0.5 break-all">{detail}</p>
                      </div>
                ...
                {addingProduct ? (
                  <div className="bg-white border-2 border-indigo-100 rounded-3xl p-5 mt-2 space-y-4 animate-in zoom-in-95 duration-200">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Nuevo producto financiero</p>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black ${
                                np.modoEnabled ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-400'
                              }`}>
                                M
                              </div>
                    ...
                    {productSegments.length > 0 && (
                      <Field label="Segmento">
                        <select value={np.segmentId} onChange={e => setNp(n => ({...n, segmentId: e.target.value}))} className={selInd}>
                          <option value="">General / Sin segmento</option>
                    ...
                    {np.entityType === 'bank' && (
                      <div>
                        <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide ml-1 mb-2 block">Tipo de producto *</label>
                        <div className="grid grid-cols-2 gap-2">
                          {(['CARD', 'ACCOUNT'] as const).map(pt => (
                            <button key={pt}
                              onClick={() => setNp(n => ({...n, productType: pt, lastFour: '', accountNumber: ''}))}
                              className={`py-3 rounded-2xl text-xs font-bold border-2 transition-all flex items-center justify-center gap-2 ${np.productType === pt ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-100 bg-gray-50 text-gray-500 hover:border-gray-200'}`}>
                    ...
                    {/* Paso 4a · TARJETA → Tipo + Marca */}
                    {np.productType === 'CARD' && (
                      <div className="grid grid-cols-2 gap-3">
                        <Field label="Tipo de tarjeta">
                          <select value={np.cardType} onChange={e => setNp(n => ({...n, cardType: e.target.value}))} className={selInd}>
                        ...
                        <div className="col-span-2 space-y-2">
                          <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide ml-1 mb-1.5 block">Marcas de Tarjeta</label>
                          <div className="grid grid-cols-1 gap-2">
                             {entities?.cardNetworks?.map(cn => {
                               const netData = np.networks.find(x => x.id === cn.id)
                               const isSelected = !!netData
                               const inModo = netData?.modo ?? false

                               return (
                                 <div 
                                   key={cn.id}
                                   className={`flex items-center gap-3 px-4 py-3 rounded-2xl border-2 transition-all cursor-pointer ${isSelected ? 'border-indigo-500 bg-indigo-50/50' : 'border-gray-50 bg-gray-50 text-gray-400 hover:border-gray-100'}`}
                                   onClick={() => toggleNetwork(cn.id)}
                                 >
                                    <div className={`w-5 h-5 rounded-lg border flex items-center justify-center shrink-0 ${isSelected ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-gray-200 text-transparent'}`}>
                                       <Plus size={12} strokeWidth={4} />
                                    </div>
                                    <span className={`text-xs font-bold uppercase tracking-tight flex-1 ${isSelected ? 'text-indigo-900' : 'text-gray-400'}`}>{cn.name}</span>
                                    
                                    {/* Mini Toggle MODO para esta tarjeta específica */}
                                    {isSelected && modoWallet && (
                                      <button
                                        type="button"
                                        onClick={(e) => toggleNetworkModo(e, cn.id)}
                                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 transition-all ${inModo ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-200 text-gray-400'}`}
                                      >
                                        <span className="text-[9px] font-black italic">MODO</span>
                                        <div className={`w-2 h-2 rounded-full ${inModo ? 'bg-white' : 'bg-gray-200'}`} />
                                      </button>
                                    )}
                                 </div>
                               )
                    ...
                    {np.productType === 'CARD' && (
                      <Field label="Últimos 4 números de la tarjeta (opcional)">
                        ...
                        />
                        {np.lastFour && (
                          <p className="text-[10px] text-gray-400 mt-1 ml-1">Se verá como: **** **** **** {np.lastFour}</p>
                        )}
                      </Field>
                    )}
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={handleAddProduct}
                        disabled={saving || (!np.bankId && !np.walletId)}
                      ...
                      <button onClick={() => { setAddingProduct(false); setEditingId(null); resetForm() }}
                        className="px-5 py-3.5 bg-gray-100 text-gray-500 rounded-2xl text-sm font-bold">
                        Cancelar
                      </button>
                    </div>
                  </div>
                  ...
                  <button onClick={() => setAddingProduct(true)}
                    className="flex items-center justify-center gap-2 text-sm w-full py-4 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-300 transition-all font-bold mt-1">
      ...
      {showSaved && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end sm:justify-center items-center bg-black/40 backdrop-blur-sm p-4 sm:p-0">
          <div className="bg-gray-50 w-full max-w-sm h-[80vh] rounded-[32px] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-200 border border-white/20">
            <div className="bg-white px-5 py-4 flex items-center justify-between border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-2">
                <Heart size={18} className="text-red-500 fill-red-500" />
                <h2 className="font-bold text-gray-900">Promos Guardadas</h2>
              </div>
              <button onClick={() => setShowSaved(false)} className="p-1.5 bg-gray-100 text-gray-500 rounded-full hover:bg-gray-200 transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="p-4 overflow-y-auto flex-1 space-y-3">
              {loadingSaved ? (
                <div className="py-10 text-center text-sm font-medium text-gray-400 animate-pulse">Cargando tus promos...</div>
                ...
                <div className="py-10 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm font-bold text-gray-900">Aún no guardaste ninguna promo</p>
                  <p className="text-xs text-gray-500 mt-1 max-w-[200px]">Tocá el corazón en las tarjetas de ahorro para tenerlas a mano acá.</p>
                </div>
                ...
                return (
                  <div key={promo.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{promo.category?.name || 'Varios'}</p>
                        <h3 className="font-bold text-gray-900 text-[15px] mt-0.5 leading-tight">{promo.commerce?.name || promo.title}</h3>
