...
export default function PerfilPage() {
  ...
  const emptyProduct = {
    ...
    cardType: 'CREDIT',
    // Cada item es una combinación de RED + TIPO + MODO
    selectedCards: [] as { networkId: string; cardType: 'CREDIT' | 'DEBIT' | 'PREPAID'; modo: boolean }[],
    lastFour: '',
    accountNumber: '',
    shortAccountNumber: '',
    bankAccountType: 'CA',
  ...
  // Nueva función: toggle una combinación de Red + Tipo
  const toggleCard = (networkId: string, cardType: 'CREDIT' | 'DEBIT' | 'PREPAID') => {
    setNp(n => {
      const exists = n.selectedCards.find(x => x.networkId === networkId && x.cardType === cardType)
      if (exists) {
        // Desmarcar
        return { ...n, selectedCards: n.selectedCards.filter(x => !(x.networkId === networkId && x.cardType === cardType)) }
      } else {
        // Marcar
        return { ...n, selectedCards: [...n.selectedCards, { networkId, cardType, modo: false }] }
      }
    })
  }

  // Toggle MODO en una card específica
  const toggleCardModo = (e: React.MouseEvent, networkId: string, cardType: 'CREDIT' | 'DEBIT' | 'PREPAID') => {
    e.stopPropagation()
    setNp(n => ({
      ...n,
      selectedCards: n.selectedCards.map(x =>
        (x.networkId === networkId && x.cardType === cardType)
          ? { ...x, modo: !x.modo }
          : x
      )
    }))
  ...
  function handleEditProduct(p: FinancialProduct) {
    ...
    setNp({
      ...
      selectedCards: p.cardNetwork?.id
        ? [{ networkId: p.cardNetwork.id, cardType: p.cardType as 'CREDIT' | 'DEBIT' | 'PREPAID', modo: isModo }]
        : [],
      lastFour: p.lastFour || '',
              ...
              <div className="flex flex-col gap-3">
                ...
                {addingProduct ? (
                  <div className="bg-white border-2 border-indigo-100 rounded-3xl p-5 mt-2 space-y-4 animate-in zoom-in-95 duration-200">
                        ...
                        {modoWallet && np.productType === 'ACCOUNT' && (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black ${np.modoEnabled ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-400'
                                }`}>
                                M
                              </div>
                    ...
                    {np.productType === 'CARD' && (
                      <div className="space-y-3">
                        <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wide ml-1 block">
                          Seleccioná tus tarjetas (podés elegir varias)
                        </label>

                        <div className="space-y-2">
                          {entities?.cardNetworks?.map(cn => {
                            // Definir qué tipos están disponibles para cada red
                            const availableTypes: ('CREDIT' | 'DEBIT' | 'PREPAID')[] = (() => {
                              const networkName = cn.name.toLowerCase()

                              // American Express: Solo crédito y compra (prepaga)
                              if (networkName.includes('american express') || networkName.includes('amex')) {
                                // Si es "American Express Banco" → solo crédito
                                if (networkName.includes('banco')) {
                                  return ['CREDIT']
                                }
                                // American Express normal → crédito y compra (prepaga)
                                return ['CREDIT', 'PREPAID']
                              }

                              // Cabal, Maestro: Crédito y Débito
                              if (networkName.includes('cabal') || networkName.includes('maestro')) {
                                return ['CREDIT', 'DEBIT']
                              }

                              // Visa, Mastercard: Los 3 tipos
                              if (networkName.includes('visa') || networkName.includes('mastercard')) {
                                return ['CREDIT', 'DEBIT', 'PREPAID']
                              }

                              // Default para redes desconocidas: los 3 tipos
                              return ['CREDIT', 'DEBIT', 'PREPAID']
                            })()
                            ...
                            const typeLabels: Record<string, string> = {
                              CREDIT: 'Crédito',
                              DEBIT: 'Débito',
                              PREPAID: 'Prepaga / Compra'
                            }

                            return (
                              <div key={cn.id} className="bg-gray-50 rounded-2xl p-3 space-y-1.5">
                                {/* Un solo título por red */}
                                ...
                                <div className="grid grid-cols-1 gap-1.5">
                                  {availableTypes.map(type => {
                                    const cardData = np.selectedCards.find(x => x.networkId === cn.id && x.cardType === type)
                                    const isSelected = !!cardData
                                    const inModo = cardData?.modo ?? false
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className={`text-xs font-bold flex-1 ${isSelected ? 'text-indigo-900' : 'text-gray-500'
                                          }`}>
                                          {typeLabels[type]}
                                        </span>
                                        ...
                                        {isSelected && modoWallet && (
                                          <button
                                            type="button"
                                            onClick={(e) => toggleCardModo(e, cn.id, type)}
                                            className={`flex items-center gap-2 px-2.5 py-1 rounded-lg border-2 transition-all ${inModo
