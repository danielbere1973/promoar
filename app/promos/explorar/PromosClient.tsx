...
export default function PromosClient({ initialPromos, initialCats, initialTotalCount, initialProvince }: { initialPromos: Promo[] | null, initialCats: string[], initialTotalCount: number, initialProvince?: string | null }) {
  ...
  // Base propia (/promos/explorar), no hardcodeada a /promos — al cerrar el
  // detalle, el usuario vuelve al catálogo, no a la Home (Home v2, 9/8/2026).
  const BASE_PATH = '/promos/explorar'

  const openPromoDetail = React.useCallback((promo: Promo) => {
    setDetailPromo(promo)
    if (promo.slug) {
      window.history.pushState({ promoSlug: promo.slug }, '', `${BASE_PATH}/${promo.slug}`)
    }
  }, [])
        ...
        {!loading && !isProductSearchMode && promosFiltradas.length > 0 && (() => {
          ...
          return (
            <div className="space-y-0 -mx-4">
              ...
              {/* Home v2 (CPO Review, 9/8/2026): Spotlight + recomendaciones + señales +
                  CTA "Explorar" ahora viven en /promos (Home cerrada). Esta ruta
                  (/promos/explorar) es el catálogo completo — se llega acá únicamente
                  a través del CTA de la Home, nunca muestra recomendaciones personalizadas. */}
