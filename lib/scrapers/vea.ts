// Vea Scraper V4
// Fuente: https://www.vea.com.ar/descuentos-del-dia?type=por-banco
...
// ─── parsePageText ────────────────────────────────────────────────────────────
// Vea SPA innerText pattern per bank page (misma estructura VTEX que Jumbo):
...
async function scrapeBankPage(
  ...
  try {
    ...
    if (fullText.length < 500) {
      console.warn(`[Vea] Sin contenido para ${bank.param}`);
      return [];
    }
    ...
    console.error(`[Vea] Error en ${bank.param}:`, err);
    return [];
  }
}

// ─── Main Scraper ─────────────────────────────────────────────────────────────
export const VeaScraper: Scraper = {
  name: STORE_NAME,

  async run(): Promise<ScrapedPromo[]> {
    console.log('[Vea] Iniciando scraper...');
    ...
    console.log(`[VeaScraper] Total: ${unique.length} promos (${allPromos.length} antes de dedup)`);
    return unique;
  },
};
