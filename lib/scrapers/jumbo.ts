// Jumbo Scraper V4
// Fuente: https://www.jumbo.com.ar/descuentos-del-dia?type=por-banco
...
const BANKS: { param: string; displayName: string; logoMatch: string; type?: 'wallet' }[] = [
  ...
  { param: 'MODO',                displayName: 'MODO',        type: 'wallet', logoMatch: 'modo' },
  ...
  { param: 'CencoPay',            displayName: 'Cencopay Mastercard',    type: 'wallet', logoMatch: 'cencopay1' },
...
const BASE_URL = 'https://www.jumbo.com.ar/descuentos-del-dia?type=por-banco';
const STORE_NAME = 'Jumbo';
...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  // Patrón más común en Jumbo: "VÁLIDA DESDE 01/04/2026 HASTA 30/04/2026"
  ...
  // Fechas individuales en texto "el 02/01/2026 y el 30/04/2026"
...
function extractCardType(text: string): 'CREDIT' | 'DEBIT' | 'PREPAID' | null {
  ...
  // TD = Tarjeta de Débito (abreviatura usada por Supervielle, etc.)
...
// ─── parsePageText ────────────────────────────────────────────────────────────
// Jumbo SPA innerText pattern per bank page:
...
// This handles blocks like Supervielle's "MARTES JUBILADOS..." that don't
// start with any recognized keyword like "PROMOCIÓN VÁLIDA" or "CARTERA DE".
function parsePageText(
  ...
  // Split on the "+" / "Ver más" pattern that Jumbo uses after each promo card's legal text.
...
// ─── scrapeBankPage ───────────────────────────────────────────────────────────
// IMPORTANTE: Jumbo es una SPA VTEX donde el filtro "?bank=X" en la URL NO
...
// contenido genérico (~450-1600 chars, la promo "3 cuotas sin interés" de
// Planes de Financiación), sin importar el banco pedido — esa era la causa de
// que el scraper perdiera casi todas las promos reales por banco. Confirmado
// en vivo el 11/8/2026: mismo click vía `locator.click()` sobre el logo trae
// 2-3x más contenido (hasta 5300 chars) con las promos reales de esa entidad.
async function scrapeBankPage(
  page: Page,
  bank: { param: string; displayName: string; logoMatch: string; type?: 'wallet' },
): Promise<ScrapedPromo[]> {
  console.log(`[Jumbo] Scrapeando ${bank.displayName} (logo~"${bank.logoMatch}")`);
  ...
  try {
    ...
    if (found === 0) {
      console.warn(`[Jumbo] Logo no encontrado en la grilla para ${bank.displayName} (sin promos activas hoy)`);
      return [];
    }
    ...
    console.log(`[Jumbo] ${bank.displayName}: ${fullText.length} chars`);

    if (fullText.length < 500) {
      console.warn(`[Jumbo] Sin contenido para ${bank.param}`);
      return [];
    }
    ...
    console.error(`[Jumbo] Error en ${bank.param}:`, err);
    return [];
  }
}

// ─── Main Scraper ─────────────────────────────────────────────────────────────
export const JumboScraper: Scraper = {
  name: STORE_NAME,

  async run(): Promise<ScrapedPromo[]> {
    console.log('[Jumbo] Iniciando scraper...');
    ...
    console.log(`[JumboScraper] Total: ${unique.length} promos (${allPromos.length} antes de dedup)`);
    return unique;
  },
};
