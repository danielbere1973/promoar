...
export const CotoScraper: Scraper = {
  ...
  async run(): Promise<ScrapedPromo[]> {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      // ── Extracción de datos ───────────────────────────────────────────────
      const { validFrom: rawValidFrom, validUntil: rawValidUntil, specificDates } = extractDates(bodyText);

      // Si hay fechas específicas, derivar validFrom/validUntil de ellas
      // para evitar inconsistencias (ej: validFrom posterior a la primera fecha específica)
      let validFrom = rawValidFrom;
      let validUntil = rawValidUntil;
      if (specificDates && specificDates.length > 0) {
        const sorted = [...specificDates].sort();
        const firstDate = sorted[0];
        const lastDate = sorted[sorted.length - 1];
        // validFrom no puede ser posterior a la primera fecha específica
        if (!validFrom || validFrom > firstDate) validFrom = firstDate;
        // validUntil no puede ser anterior a la última fecha específica
        if (!validUntil || validUntil < lastDate) validUntil = lastDate;
      }
