...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  // Normalizar sin tildes ni minúsculas para simplificar regex
  const normText = text.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');

  // ── 1. Lista de fechas numéricas: "06/04/2026, 13/04/2026 Y 27/04/2026" ──────
  ...
  // ── 2. Rango numérico completo: "DEL 01/04/2026 AL 30/04/2026" ────────────────
  ...
  // ── 3. Fechas específicas nombradas ──────────────────────────────────────────
  // Cubre todos estos patrones:
  //   "SÁBADO 18, DOMINGO 19 Y LUNES 20 DE ABRIL DE 2026"  → [18,19,20]
  //   "JUEVES 02 DE ABRIL Y JUEVES 16 DE ABRIL DE 2026"     → [02,16]
  //   "SABADO 04 DE ABRIL HASTA EL DOMINGO 05 DE ABRIL"      → [04,05]
  // Excluye rangos como "DEL 1 AL 30 DE ABRIL" (el 30 está precedido por "AL")
  const simplified = normText.replace(/\b(LUNES|MARTES|MIERCOLES|JUEVES|VIERNES|SABADO|DOMINGO)\b/g, '');
  const specificDatesFound: string[] = [];

  // Buscar grupos "N1, N2... DE MES [DE AÑO]"
  const groupMatches = execAll(/(\d{1,2}(?:\s*[,Y]\s*\d{1,2})*)\s+DE\s+([A-Z]+)(?:\s+DE\s+(\d{4}))?/g, simplified);
  for (const gm of groupMatches) {
    // Saltar si el número está precedido por "AL" (es fin de rango, no fecha específica)
    const matchStart = gm.index!;
    const precedingText = simplified.substring(Math.max(0, matchStart - 6), matchStart);
    if (/\bAL\s*$/.test(precedingText)) continue;

    const monthName = gm[2].toLowerCase();
    const month = MONTHS[monthName];
    if (!month) continue;
    const year = gm[3] ? parseInt(gm[3]) : currentYear;

    const nums = execAll(/\d{1,2}/g, gm[1])
      .map(m => parseInt(m[0]))
      .filter(n => n >= 1 && n <= 31);
    for (const d of nums) {
      const iso = `${year}-${String(month).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
      if (!specificDatesFound.includes(iso)) specificDatesFound.push(iso);
    }
  }

  if (specificDatesFound.length > 0) {
    return { specificDates: specificDatesFound };
  }

  // ── 4. Rango simple: "DEL 1 AL 30 DE ABRIL [DE 2026]" ───────────────────────
  ...
  // ── 5. Rango en español: "DESDE EL 1 DE ABRIL AL 30 DE ABRIL DE 2026" ────────
