...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  // Normalizar sin tildes para simplificar regex
  ...
  // ── PASO 1: Buscar fechas específicas nombradas PRIMERO ───────────────────────
  // Cubre: "SÁBADO 25 DE ABRIL Y DOMINGO 26 DE ABRIL"
  //        "DESDE EL 18 HASTA EL 19 DE ABRIL DE 2026"
  //        "SÁBADO 18, DOMINGO 19 Y LUNES 20 DE ABRIL DE 2026"
  //        "JUEVES 02 DE ABRIL Y JUEVES 16 DE ABRIL DE 2026"
  const specificDates: string[] = [];

  // Patrón A: "DESDE EL N HASTA EL N DE MES [DE AÑO]"
  const desdeHastaMatch = normText.match(
    /DESDE\s+EL\s+(\d{1,2})\s+HASTA\s+EL\s+(\d{1,2})\s+DE\s+([A-Z]+)(?:\s+DE\s+(\d{4}))?/
  );
  if (desdeHastaMatch) {
    const monthName = desdeHastaMatch[3].toLowerCase();
    const month = MONTHS[monthName];
    const y = desdeHastaMatch[4] ? parseInt(desdeHastaMatch[4]) : currentYear;
    if (month) {
      const mm = String(month).padStart(2, '0');
      specificDates.push(`${y}-${mm}-${desdeHastaMatch[1].padStart(2, '0')}`);
      specificDates.push(`${y}-${mm}-${desdeHastaMatch[2].padStart(2, '0')}`);
    }
  }

  // Patrón B: "N DE MES [HASTA EL] [Y] N DE MES" — múltiples fechas con el mismo mes
  // Quitar nombres de días para simplificar
  const stripped = normText.replace(/\b(LUNES|MARTES|MIERCOLES|JUEVES|VIERNES|SABADO|DOMINGO)\b/g, ' ');

  // Encontrar todos los "N DE MES [DE AÑO]"
  const singleDateMatches = execAll(/(\d{1,2})\s+DE\s+([A-Z]+)(?:\s+DE\s+(\d{4}))?/g, stripped);
  for (const m of singleDateMatches) {
    const dayNum = parseInt(m[1]);
    if (dayNum < 1 || dayNum > 31) continue;
    const monthName = m[2].toLowerCase();
    const month = MONTHS[monthName];
    if (!month) continue;
    // Saltar si precedido por "AL " (es fin de rango numérico)
    const precedingText = stripped.substring(Math.max(0, m.index! - 5), m.index!);
    if (/\bAL\s*$/.test(precedingText)) continue;
    const y = m[3] ? parseInt(m[3]) : currentYear;
  ...
  // ── PASO 2: Lista de fechas numéricas "06/04/2026, 13/04/2026 Y 27/04/2026" ──
  ...
  // Rango numérico completo: "DEL 01/04/2026 AL 30/04/2026"
  ...
  // Rango simple: "DEL 1 AL 30 DE ABRIL [DE 2026]"
  ...
  // Rango español: "DESDE EL 1 DE ABRIL AL 30 DE ABRIL DE 2026"
