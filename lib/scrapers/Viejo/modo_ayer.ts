...
async function fetchPromoDetails(promoUrl: string): Promise<PromoDetails | null> {
  try {
    ...
    const paymentMethodMatch = html.match(/"paymentMethodListWithTags":\s*(\[[\s\S]*?\])\s*(?:,|})/)

    if (paymentMethodMatch) {
      try {
        // Limpiar el JSON extraído (eliminar escapes innecesarios)
        let jsonStr = paymentMethodMatch[1]
          .replace(/\\\\/g, '\\')
          .replace(/\\"/g, '"')
        ...
        console.error(`[MODO HTML] ⚠️  Error parseando paymentMethodListWithTags:`, err)
...
export const ModoScraper: Scraper = {
  ...
  async run(categoria?: string): Promise<ScrapedPromo[]> {
    ...
    console.log(`[MODO DEBUG] filteredCards.length:`, filteredCards.length);
    console.log(`[MODO DEBUG] Primeras 3 cards:`, filteredCards.slice(0, 3).map(c => c.slug));
    ...
    for (let i = 0; i < filteredCards.length; i++) {
      ...
      if (!discountInfo) {
        console.log(`[MODO DEBUG] ⚠️  Sin descuento, saltando promo`);
        continue;
      }
