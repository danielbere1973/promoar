...
async function fetchPromoDetails(promoUrl: string): Promise<PromoDetails | null> {
  try {
    ...
    // ← AGREGAR ACÁ
    const bcraIdx = html.indexOf('bcra_code');
    if (bcraIdx > -1) {
      console.log(`[MODO HTML BCRA] Contexto:`, html.substring(Math.max(0, bcraIdx - 300), bcraIdx + 300));
    } else {
      console.log(`[MODO HTML BCRA] No encontrado "bcra_code" en ${promoUrl}`);
    }
    ...
    if (banksArrayMatch) {
      ...
      if (nameMatches.length > 0 && nameMatches.length === bcraMatches.length) {
        for (let i = 0; i < nameMatches.length; i++) {
          const bankName = (nameMatches[i] as RegExpMatchArray)[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').trim();
          const bcraCode = (bcraMatches[i] as RegExpMatchArray)[1];
          if (bankName) {
        ...
        for (const match of bcraMatches) {
          const bcraCode = (match as RegExpMatchArray)[1];
