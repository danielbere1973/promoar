...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  // 2. Rango numérico: "VÁLIDO DEL 01/04/2026 AL 30/04/2026"
  ...
  // 3. Rango en español: "DESDE EL 1 DE ABRIL AL 30 DE ABRIL DE 2026"
  // FIX: antes solo devolvía validUntil, ahora devuelve validFrom también.
  const rangeMatch = text.match(
    /(?:DESDE\s+EL|DEL)\s+(\d{1,2})(?:\s+DE)?\s+(\w+)(?:\s+(?:DE)?\s*(\d{4}))?\s+(?:AL|HASTA\s+EL)\s+(\d{1,2})(?:\s+DE)?\s+(\w+)(?:\s+(?:DE)?\s*(\d{4}))?/i
  );
  if (rangeMatch) {
  ...
  if (blockMatch) {
    ...
    if (monthGlobal) {
      ...
      const dayNumbers = execAll(/\b(\d{1,2})\b/g, beforeMes)
        .map(m => parseInt(m[1]))
        .filter(n => n >= 1 && n <= 31);
      console.log(`[COTO DATES] beforeMes: "${beforeMes}"`);  // ← acá
      console.log(`[COTO DATES] dayNumbers: ${JSON.stringify(dayNumbers)}`);  // ← y acá
      for (const d of dayNumbers) {
        const iso = `${anioGlobal}-${String(monthGlobal).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
        if (!specificMatches.includes(iso)) specificMatches.push(iso);
      }
      // console.log(`[COTO DATES] blockText: ${blockMatch[1].substring(0, 150)}`);
      // console.log(`[COTO DATES] specificMatches: ${JSON.stringify(specificMatches)}`);
      console.log(`[COTO DATES] blockText normalizado: ${blockText.substring(0, 150)}`);
      console.log(`[COTO DATES] mesGlobal: ${mesGlobal}, monthGlobal: ${monthGlobal}`);
      console.log(`[COTO DATES] specificMatches: ${JSON.stringify(specificMatches)}`);
    }
