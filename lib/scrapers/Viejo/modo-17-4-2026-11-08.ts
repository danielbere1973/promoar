// MODO Scraper V3 con extracción de redes de tarjeta
// FIX: Matching de bancos usando codigoModo + American Express detection
  ...
  try {
    ...
    if (nameBankMatches.length > 0 && nameBankMatches.length === bcraCodeMatches.length) {
      for (let i = 0; i < nameBankMatches.length; i++) {
        ...
        // FIX: No agregar ceros a la izquierda, devolver código tal cual (4 dígitos)
        const bcraCode = bcraCodeMatches[i][1];
      ...
      for (const match of bcraCodeMatches) {
        // FIX: No agregar ceros a la izquierda
    ...
    if (paymentMethodMatch) {
      try {
        ...
        for (const method of methods) {
          ...
          if (name === 'visa') networks.add('Visa');
          else if (name === 'master' || name === 'mastercard') networks.add('Mastercard');
          // FIX: Detectar "american express" correctamente
          else if (name === 'amex' || name === 'american express') networks.add('American Express');
          else if (name === 'cabal') networks.add('Cabal');
          else if (name === 'maestro') networks.add('Maestro');
          else if (name === 'diners') networks.add('Diners');
        }

        details.cardNetworks = Array.from(networks);
        if (details.cardNetworks.length > 0) {
          console.log(`[MODO HTML] ✅ Redes extraídas: ${details.cardNetworks.join(', ')}`);
        }
      } catch (err) {
...
export const ModoScraper: Scraper = {
  ...
  async run(categoria?: string): Promise<ScrapedPromo[]> {
    ...
    for (let i = 0; i < filteredCards.length; i++) {
      ...
      console.log(`[MODO DEBUG] ${card.slug} → discount:`, discountInfo);
