...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  const blockMatch = text.match(
    /(?:V[AÁ]LID[AO]|VIGENCIA|VIGENTE)[^.]*?(?:EL?|LOS?)\s+D[ÍI]AS?\s+([^.]{5,200})/i
  );
  if (blockMatch) {
    const blockText = blockMatch[1].toUpperCase();

    // Multi-día: "11, 12 Y 13 DE MAYO DE 2026"
    const multiDayMatch = blockText.match(
      /((?:\d{1,2}(?:\s*,\s*|\s+Y\s+))+\d{1,2})\s+DE\s+(\w+)(?:\s+DE\s+(\d{4}))?/i
    );
    if (multiDayMatch) {
      const days = multiDayMatch[1].split(/[,y]/i).map(d => d.trim()).filter(Boolean);
      const monthName = multiDayMatch[2].toLowerCase();
      const month = MONTHS[monthName];
      const y = multiDayMatch[3] ? parseInt(multiDayMatch[3]) : currentYear;
      if (month) {
        for (const d of days) {
          const iso = `${y}-${String(month).padStart(2, '0')}-${String(parseInt(d)).padStart(2, '0')}`;
          if (!specificMatches.includes(iso)) specificMatches.push(iso);
        }
      }
    }

    // Fechas individuales: "X DE MES" o "X Y Y DE MES"
