...
function extractDates(text: string): { validFrom?: string; validUntil?: string; specificDates?: string[] } {
  ...
  // ── PASO 1: Fechas específicas ────────────────────────────────────────────────
  // Estrategia: quitar nombres de días, luego buscar cada "DE MES [DE AÑO]"
  // y recolectar TODOS los números que la preceden en el mismo grupo.
  ...
  // Encontrar todos los "DE MES [DE AÑO]" en el texto
  ...
  if (specificDates.length > 0) {
    // Consistencia: validFrom = primera fecha, validUntil = última fecha
    return {
      specificDates,
