...
// ─── scrapeBankPage ───────────────────────────────────────────────────────────
// IMPORTANTE: Disco es una SPA VTEX donde el filtro "?bank=X" en la URL NO
...
// corregido en jumbo.ts (11/8/2026), Disco comparte la misma plataforma.
async function scrapeBankPage(
  page: Page,
  bank: { param: string; displayName: string; logoMatch: string; type?: string },
): Promise<ScrapedPromo[]> {
  console.log(`[Disco] Scrapeando ${bank.displayName} (logo~"${bank.logoMatch}")`);
  ...
  try {
    ...
    if (found === 0) {
      console.warn(`[Disco] Logo no encontrado en la grilla para ${bank.displayName} (sin promos activas hoy)`);
      return [];
    }
