...
export function detectCategoria(text: string): string {
  const t = normStr(text);
  if (/JUMBO|CARREFOUR|DISCO|COTO|VEA|WALMART|CHANGO|DIARCO|SUPERMERCADO|ANONIMA|YAGUAR|MAXI/.test(t)) return 'Supermercados';
  if (/FARMACIA|PERFUMER|FARMACITY|DROGUERIA/.test(t)) return 'Farmacias';
  if (/\bYPF\b|\bSHELL\b|\bAXION\b|\bPETROBRAS\b|\bWICO\b|\bGULF\b|NAFTA|COMBUSTIBLE|ESTACION\s+DE\s+SERVICIO|SURTIDOR/.test(t)) return 'Combustible';
  if (/CONCESIONARIA|AUTOMOVIL|AUTOMOTOR(?!A)|0KM|CERO\s+KM|REPUESTO|PATENTAMIENTO|TALLER\s+MEC|DEALER|PEUGEOT|VOLKSWAGEN|\bFORD\b|TOYOTA|CHEVROLET|RENAULT|\bFIAT\b|\bHONDA\b|\bNISSAN\b|\bJEEP\b|HYUNDAI/.test(t)) return 'Automotores';
  if (/HELADERIA|HELADOS|FREDDO|CHUNGO|GRIDO|VOLTA|CREMOLATTI/.test(t)) return 'Heladerías';
  if (/HOTEL|VUELO|AEROLINEA|AEROLINEAS\s+ARG|LATAM|FLYBONDI|JETSMART|DESPEGAR|ALMUNDO|BOOKING|EDREAMS|AGENCIA\s+DE\s+VIAJE|CRUCERO|PAQUETE\s+TURI|DRAGONPASS|ACCOR|MARRIOTT|HILTON/.test(t)) return 'Viajes y Turismo';
  if (/\bUBER\b|\bCABIFY\b|SUBTE|COLECTIVO|\bTAXI\b|TELEPASE|PEAJE|TRANSPORTE\s+PUBLI|SUBE|TRENES\s+ARG/.test(t)) return 'Transporte';
  if (/RESTAURANT|PIZZA|BURGER|SUSHI|GASTRONOM|COMIDA|CAFE|BAR(?!\w)|BARES|BODEGON|PEDIDOS|RAPPI|DELIVERY|MCDONALD|STARBUCKS|MOSTAZA|PARRILLA/.test(t)) return 'Gastronomía';
  if (/ELECTRO|GARBARINO|FRAVEGA|MUSIMUNDO|CELULAR|PC(?!\w)|NOTEBOOK|LENOVO|TECNOLOG|SAMSUNG|APPLE|IPHONE|COMPUTO/.test(t)) return 'Tecnología';
  if (/ROPA|MODA|ZAPATILLAS|CALZADO|INDUMENTARIA|ZARA|H&M|MIMO|KEVINGSTON|WRANGLER|LEVIS/.test(t)) return 'Indumentaria';
  if (/ADIDAS|NIKE|PUMA|REEBOK|UNDER\s+ARMOUR|BICICLETA|CICLO|DEPORTE|FITNESS|GYM|GIMNASIO|RUNNING|PILATES|YOGA|DECATHLON/.test(t)) return 'Deportes';
  if (/PETSHOP|PETCO|VETERINAR|MASCOTA|ZOOMUNDO|PUPPIS|CATYCAN|NATURAL\s+LIFE|PET\s+CITY/.test(t)) return 'Petshops';
  if (/HELADERIA|COLCHON|SOMMIER|MUEBLE|DECORACION|HOGAR|EASY(?!\w)|SODIMAC|FALABELLA|IKEA|PINTURA|FERRETERI/.test(t)) return 'Hogar';
  if (/CINE|TEATRO|ENTRADAS|TICKETEK|SHOW|EVENTO|ENTRETENIMIENTO|NETFLIX|SPOTIFY|DISNEY/.test(t)) return 'Entretenimiento';
  if (/OPTICA|BELLEZA|ESTETICA|PELUQUERIA|SPA|CLINICA|MEDIC|SALUD(?!\s+Y\s+BELLEZA)|DENTAL/.test(t)) return 'Salud y Belleza';
  if (/JUGUETE|TOYS|BEBE\s+MUNDO|BABY/.test(t)) return 'Jugueterías';
  if (/LIBRERIA|LIBRO|EDITORIAL|CUSPIDE|EL\s+ATENEO/.test(t)) return 'Librerías';
  if (/SHOPPING|ALTO\s+PALERMO|DOT\s+BAIRES|UNICENTER|ABASTO|PATIO\s+BULL/.test(t)) return 'Shoppings';
  return '';
}
