...
export function detectCategoria(text: string): string {
  const t = normStr(text);
  if (/JUMBO|CARREFOUR|DISCO|COTO|VEA|WALMART|CHANGO|DIARCO|SUPERMERCADO|ANONIMA|YAGUAR|MAXI|LA\s+GALLEGA|HIPERMAXI|CHANGOMAS|WALMART|BELL'S|BELLS|COOPERATIVA|SUPER\s+VEA|LIDER|EKONO|WALMART/.test(t)) return 'Supermercados';
  ...
  if (/RESTAURANT|PIZZA|BURGER|SUSHI|GASTRONOM|COMIDA|CAFE|BAR(?!\w)|BARES|BODEGON|PEDIDOS|RAPPI|DELIVERY|MCDONALD|STARBUCKS|MOSTAZA|PARRILLA|CARNICER|FIAMBRE|QUESERIA|QUESO\s|MEDIALUNA|CHURRO|EMPANADA|PICADA|ALMACEN\s+DE|GRANJA|ROTISERIA|VIANDAS/.test(t)) return 'Gastronomía';
